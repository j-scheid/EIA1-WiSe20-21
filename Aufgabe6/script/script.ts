...
function click() {
    ...
    for (let f = 0; f < img.length; f++) {
      img[f].addEventListener("click", function() {
        ...
        //Berechnung
        let relative = (continent[f].e18 / total18 * 100).toFixed(2);
        let growthpercent = (continent[f].e18 / continent[f].e08 * 100 - 100).toFixed(2);    
        let growthabsolute = (continent[f].e18 - continent[f].e08).toFixed(2);
